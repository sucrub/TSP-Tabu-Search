...
void tabuSearch()
{
	...
	while (!stop)
	{
			...
			for (int j = 1; j < N; j++)
			{
				...
				if (tabu_list[i] == 0 && tabu_list[j] == 0)
				{
					...
					if (fitness(tmp_sol) < cmp)
					{
						...
						// Update the value
						for (int i = 0; i < N; i++)
							neighbor_candidate[i] = tmp_sol[i];
				...
				else
				{
					...
					if (fitness(tmp_sol) < best_val && fitness(tmp_sol) < cmp)
					{
		...
		{ // If this solution is better than what we have so far

			for (int i = 0; i < N; i++)
				best_sol[i] = best_candidate[i];
